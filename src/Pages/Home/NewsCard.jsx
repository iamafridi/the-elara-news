...
const NewsCard = ({ news }) => {
    ...
    const { title, category_id,image_url, total_view, author, details } = news;
    return (
        <div>
            <div className="flex flex-col max-w-lg p-6 space-y-6 overflow-hidden rounded-lg shadow-md dark:bg-gray-900 dark:text-gray-100">
                ...
                <div>
                    ...
                    {
                        details.length > 200 ? <p>{details.slice(0, 200)} <br /><p className="text-end mt-2"><Link 
                        to={`/news/${category_id}`}
                        className="text-red-400 font-semibold" >Read More...</Link></p></p>
                            :
                            <p>{details}</p>
                    }
                </div>
